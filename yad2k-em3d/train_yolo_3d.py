#! /usr/bin/env python
"""Train a 3D YOLO network (v2) using custom data."""
import argparse
import os

from models.keras_yolo_3d import create_model, train
import utils

parser = argparse.ArgumentParser(description="Train a 3D YOLO network (v2) using custom data.")
parser.add_argument(
    '-a', '--anchors_path',
...
parser.add_argument(
    '-d', '--data_path',
    help="path to numpy data file (.npz) containing np.object array 'boxes' and np.uint8 array 'images', defaults to images/spheres.npz",
    default=os.path.join('images', 'spheres.npz'))
parser.add_argument(
    '-o', '--output_path',
    help='path to prefix of trained models, defaults to training/spheres',
    default=os.path.join('training', 'spheres'))

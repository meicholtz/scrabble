...
outputfile = '\\sshfs\mve@deepblue\git\yad2k\testing\spheres_overfit_large_batch1_stage3.mat';
...
%     bbox(all(bbox==0,2),:) = [];
    ...
    position = bsxfun(@times,bbox(:,1:3),fliplr(size(I)));
    shape = bsxfun(@times,bbox(:,4:6),fliplr(size(I)));
    label = bbox(:,end);

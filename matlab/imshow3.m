...
switch ndims(I)
    ...
    case 4
        [row,col,~,layers] = size(I);
    otherwise
        error('Invalid number of dimensions in the input image volume: %d',ndims(I));
end
...
text(col-4,-24,sprintf('%d/%d',1,layers),'FontSize',12,'FontWeight','bold','Color','k',...
    'Interpreter','none','HorizontalAlignment','right','VerticalAlignment','top','Tag','counter');
hold off;
axis off;
...
%UPDATEFIGURE Update the figure when the slider value changes.
    ...
    htext.String = sprintf('%d/%d',ind,layers);
end
